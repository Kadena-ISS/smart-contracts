...
import type { ICommand, IKeypair } from '@kadena/client';
import { Pact, createClient, createSignWithKeypair } from '@kadena/client';
import fs from 'fs';
import path from 'path';
import { retrieveContractFromChain } from '@kadena/pactjs-cli/lib/utils/retrieveContractFromChain'
import { devnetUrl, mainNetHost, networkId } from '../../../constants/network.constants'
...
export const deployGaurdsContract = async ({
  ...
  const contract = fs.readFileSync(
    path.resolve(__dirname, './../guards.pact'),
    'utf-8',
  );
  ...
  const signWithKeypair = createSignWithKeypair([sender00Account.keys[0]] as IKeypair[]);
